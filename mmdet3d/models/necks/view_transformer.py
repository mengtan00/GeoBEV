...
class LSSViewTransformer(BaseModule):
    ...
    def create_grid_infos(self, x, y, z, **kwargs):
        ...
        """
        self.grid_lower_bound = torch.Tensor([cfg[0] for cfg in [x, y, z]]).cuda()
        self.grid_interval = torch.Tensor([cfg[2] for cfg in [x, y, z]]).cuda()
        self.grid_size = torch.Tensor([(cfg[1] - cfg[0]) / cfg[2]
                                       for cfg in [x, y, z]]).cuda()
    ...
    def voxel_pooling_prepare_v2(self, coor):
        ...
        coor = ((coor - self.grid_lower_bound) /
                self.grid_interval)
        coor = coor.long().view(num_points, 3)
        batch_idx = torch.range(0, B - 1, device=coor.device).reshape(B, 1). \
