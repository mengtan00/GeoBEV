...
class MEGVIIEMAHook(Hook):
    ...
    def __init__(self, init_updates=0, decay=0.9990, resume=None, save_iter=20000):
        super().__init__()
    ...
    def after_train_iter(self, runner):
        ...
        if runner.inner_iter!=0 and runner.inner_iter % self.save_iter == 0:
    ...
    def save_checkpoint_iter(self, runner):
        ...
        save_path = f'epoch_{runner.epoch+1}_{runner.inner_iter}_ema.pth'
        save_path = os.path.join(runner.work_dir, save_path)
        torch.save(ema_checkpoint, save_path)
        runner.logger.info(f'Saving ema checkpoint at {save_path}')
        ...
        save_path = f'epoch_{runner.epoch+1}_{runner.inner_iter}.pth'
        save_path = os.path.join(runner.work_dir, save_path)
        torch.save(checkpoint, save_path)
        runner.logger.info(f'Saving checkpoint at {save_path}')
