...
def gaussian_2d_gpu(shape, sigma=1):
    """Generate gaussian map.
    ...
    """
    m, n = [(ss - 1.) / 2. for ss in shape]
    x = torch.arange(-m, m + 1, device='cuda').view(1, shape[0])
    y = torch.arange(-n, n + 1, device='cuda').view(shape[1], 1)
...
def draw_heatmap_gaussian(heatmap, center, radius, k=1):
    ...
    # gaussian = gaussian_2d_gpu((diameter, diameter), sigma=diameter / 6)
    ...
    #              radius - left:radius + right].to(torch.float32)
